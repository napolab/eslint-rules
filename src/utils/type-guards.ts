...
export function isFunctionDeclarationWithId(node: Rule.Node): node is Rule.Node & {
  ...
  return node.type === "FunctionDeclaration" && Boolean(node.id) && "name" in node.id;
}
...
export function isVariableDeclaratorWithId(node: Rule.Node): node is Rule.Node & {
  ...
  return node.type === "VariableDeclarator" && Boolean(node.id) && node.id.type === "Identifier" && "name" in node.id;
}
...
export function isExportSpecifierWithLocal(node: Rule.Node): node is Rule.Node & {
  ...
  return (
    node.type === "ExportSpecifier" && Boolean(node.local) && node.local.type === "Identifier" && "name" in node.local
  );
}
